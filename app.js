...
function cardTemplate(item, index, opts = {}) {
  ...
  const flag = item.flags?.new
    ? '<div class="badge-flag">NOUVEAU</div>'
    : item.flags?.top
    ? '<div class="badge-flag">TOP</div>'
    : "";
...
function renderRow(container, items, opts = {}) {
  container.innerHTML = `
    ...
    <div class="scroller">${items
      .map((it, i) => cardTemplate(it, i, opts))
      .join("")}</div>
  ...
  qs(".nav-prev", container).addEventListener("click", () =>
    scroller.scrollBy({ left: -step(), behavior: "smooth" })
  );
  qs(".nav-next", container).addEventListener("click", () =>
    scroller.scrollBy({ left: step(), behavior: "smooth" })
  );
}
      ...
      playerHTML = `
        <div class="episode-list">
          ${episodes
            .map(
              (ep, i) =>
                `<button class="ep-btn" data-embed="${ep.embed}">${ep.title}</button>`
            )
            .join("")}
        </div>
  ...
  qsa(".ep-btn", overlay).forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      qs(
        "#player-zone",
        overlay
      ).innerHTML = `<iframe src="${epEmbed}" allowfullscreen allow="autoplay" style="width:100%;height:360px;border:0;border-radius:12px;"></iframe>`;
    });
  });
    ...
    playBtn.addEventListener(
      ...
      () => {
        qs(
          "#player-zone",
          overlay
        ).innerHTML = `<iframe src="${embedData}" allowfullscreen allow="autoplay" style="width:100%;height:360px;border:0;border-radius:12px;"></iframe>`;
      },
      { once: true }
    );
  }
